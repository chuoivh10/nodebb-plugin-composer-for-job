...
plugin.init = function(data, callback) {
	...
	data.router.get('/admin/plugins/composer-for-job', data.middleware.admin.buildHeader, controllers.renderAdminPage);
	data.router.get('/api/admin/plugins/composer-for-job', controllers.renderAdminPage);

	callback();
};

plugin.appendConfig = function(config, callback) {
	meta.settings.get('composer-for-job', function(err, settings) {
		if (err) {
			return callback(null, config);
		}

		config['composer-for-job'] = settings;
...
plugin.addAdminNavigation = function(header, callback) {
	header.plugins.push({
		route: '/plugins/composer-for-job',
		icon: 'fa-edit',
